...
# Taken from https://github.com/mlyg/unified-focal-loss, Repository for the code used in
# "Unified Focal Loss: Generalising Dice and Cross Entropy-based Losses to Handle Class
# Imbalanced Medical Image Segmentation".

##To use each loss function, they can be called in model.compile
##where the hyperparameters can be specified (otherwise default values used):
##For example: model.compile(loss = asym_unified_focal_loss(), ...)


# Helper function to enable loss function to be flexibly used for 
# both 2D or 3D image segmentation - source: https://github.com/frankkramer-lab/MIScnn
def identify_axis(shape):
    return [1,2,3] #hard coded fix, assumes always 3D segmentation...
##    # Three dimensional
...
def tversky_loss(delta = 0.7, smooth = 0.000001):
    ...
    smooth : float, optional
        smoothing constant to prevent division by zero errors, by default 0.000001
    """
    def tversky_loss_function(y_true, y_pred):
        axis = identify_axis(y_true.get_shape())
        # Calculate true positives (tp), false negatives (fn) and false positives (fp)   
        tp = K.sum(y_true * y_pred, axis=axis)
        fn = K.sum(y_true * (1-y_pred), axis=axis)
...
def dice_coefficient(delta = 0.5, smooth = 0.000001):
    ...
    smooth : float, optional
        smoothing constant to prevent division by zero errors, by default 0.000001
    """
    def dice_coefficient_loss_function(y_true, y_pred):
        axis = identify_axis(y_true.get_shape())
        # Calculate true positives (tp), false negatives (fn) and false positives (fp)   
        tp = K.sum(y_true * y_pred, axis=axis)
        fn = K.sum(y_true * (1-y_pred), axis=axis)
    ...
    return dice_coefficient_loss_function
...
def combo_loss(alpha=0.5,beta=0.5):
    ...
    """
    def combo_loss_function(y_true,y_pred):
...
def focal_tversky_loss(delta=0.7, gamma=0.75, smooth=0.000001):
    ...
    """
    def focal_tversky_loss_function(y_true, y_pred):
        # Clip values to prevent division by zero error
        epsilon = K.epsilon()
        y_pred = K.clip(y_pred, epsilon, 1. - epsilon) 
        axis = identify_axis(y_true.get_shape())
...
def symmetric_focal_loss(delta=0.7, gamma=2.):
    ...
    """
    def symmetric_focal_loss_function(y_true, y_pred):
    ...
    return symmetric_focal_loss_function
...
def symmetric_focal_tversky_loss(delta=0.7, gamma=0.75):
    ...
    gamma : float, optional
        focal parameter controls degree of down-weighting of easy examples, by default 0.75
    """
    def symmetric_focal_tversky_loss_function(y_true, y_pred):
        # Clip values to prevent division by zero error
        epsilon = K.epsilon()
        y_pred = K.clip(y_pred, epsilon, 1. - epsilon)
    ...
    return symmetric_focal_tversky_loss_function
...
def asymmetric_focal_loss(delta=0.7, gamma=2.):
    def asymmetric_focal_loss_function(y_true, y_pred):
...
##    """
    ...
    return asymmetric_focal_loss_function
...
def asymmetric_focal_tversky_loss(delta=0.7, gamma=0.75):
    ...
    gamma : float, optional
        focal parameter controls degree of down-weighting of easy examples, by default 0.75
    """
    def asymmetric_focal_tversky_loss_function(y_true, y_pred):
        # Clip values to prevent division by zero error
        epsilon = K.epsilon()
        y_pred = K.clip(y_pred, epsilon, 1. - epsilon)
    ...
    return asymmetric_focal_tversky_loss_function
...
def sym_unified_focal_loss(weight=0.5, delta=0.6, gamma=0.5):
    ...
    """
    def sym_unified_focal_loss_function(y_true,y_pred):
    ...
    return sym_unified_focal_loss_function
...
def asym_unified_focal_loss(weight=0.5, delta=0.6, gamma=0.5):
    ...
    def asym_unified_focal_loss_function(y_true,y_pred):
    ...
    return asym_unified_focal_loss_function
